...
- On `tools/call` transport failure: the executor reports the failure to the supervisor, which closes the dead generation and re-enters the same backoff loop, so a server that died between calls reconnects automatically instead of failing every future call. Business failures the server reported as `isError: true` never trigger a reconnect.
- **Initial-connection failures share the same budget**: the first connect is attempt 1 of `maxAttempts`, and a never-connected server that exhausts the cap stays down until an HMR reload or Host restart. Set a large `maxAttempts` in `reconnect` for machines where the server reliably comes up after the harness (e.g. at logon).
